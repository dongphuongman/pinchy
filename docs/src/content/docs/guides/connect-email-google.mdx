...
<Aside type="note">
  ...
  **Connected apps** section and each mailbox in the **Integrations** list below
  it. See [Connect
...
The Google OAuth app you set up in Part 1 lives in the **Connected apps** section at the top of **Settings → Integrations**. You can update or reset it at any time — the app and the connected mailboxes are managed separately.

### Edit the app credentials

Click **Edit** on the Google row to update the Client ID or Client Secret. The Client ID stays pre-filled; the Client Secret field is empty and left unchanged unless you type a new one. Changes apply to every Gmail mailbox connected through this app.
...
To recover, open the mailbox's `…` menu and click **Reconnect**. This restarts the Google sign-in flow for that mailbox and keeps its existing agent permissions attached — you don't have to re-grant anything.
