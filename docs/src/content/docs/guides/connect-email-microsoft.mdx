...
Click **Edit** on the Microsoft row to update the Client ID, Client Secret, or Tenant ID. The Client ID and Tenant ID stay pre-filled; the Client Secret is never displayed and must be pasted before you can save — enter the new secret (or re-enter the current one if you're only changing the Client ID or Tenant ID). Changes apply to every Microsoft 365 mailbox connected through this app.
