...
Two things to know. Links handed out **before** this upgrade cannot be checked — there is no honest way to reconstruct what they were given, and guessing from what is on disk today would certify exactly the swap this prevents — so they keep working the way they did. And a link whose file really is replaced or deleted now stops working instead of serving whatever took its place: ask the agent for the file again and you get a fresh one. That is also what `pinchy_delete` above runs into — an agent retiring a file it wrote ends the download link that file was handed out on.
