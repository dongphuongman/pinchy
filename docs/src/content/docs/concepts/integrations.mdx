...
### When the OAuth app itself goes missing

Email connections (Gmail and Microsoft 365) depend on a separate, shared OAuth app — see [The app and the mailboxes](#the-app-and-the-mailboxes). If an admin resets or deletes that app's credentials in **Connected apps** while mailboxes are still connected through it, each affected mailbox shows an amber **"App not configured"** badge on **Settings → Integrations**, distinct from the red **"Authentication failed"** state used for revoked or expired credentials.
